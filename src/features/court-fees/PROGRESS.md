# Court Fee Calculator — Implementation Progress

> This tracker documents the implementation status of each state calculator.
> **Implemented** = code exists and passes tests.
> **Tested** = unit tests exist and pass.
> **Statute Verified** = implementation verified against the authoritative statutory schedule.

| State | Code | Implemented | Tested | Statute Verified |
|-------|------|-------------|--------|------------------|
| Andhra Pradesh | AP | ✅ | ✅ | ✅ |
| Telangana | TG | ✅ | ✅ | ✅ |
| Delhi | DL | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Chandigarh | CH | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Karnataka | KA | ✅ | ✅ | ✅ |
| Tamil Nadu | TN | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Maharashtra | MH | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Kerala | KL | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Gujarat | GJ | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| West Bengal | WB | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Uttar Pradesh | UP | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Madhya Pradesh | MP | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Haryana | HR | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Punjab | PB | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Odisha | OR | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Bihar | BR | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Uttarakhand | UT | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Jharkhand | JH | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Chhattisgarh | CT | ❌ | ❌ | ❌ |
| Rajasthan | RJ | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Assam | AS | ❌ | ❌ | ❌ |
| Himachal Pradesh | HP | ✅ | ✅ | ❌ (Century Law Firm parity only) |
| Jammu and Kashmir | JK | ✅ | ✅ | ❌ (Century Law Firm parity only) |
