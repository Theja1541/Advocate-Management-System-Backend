...
describe('TG Court Fee Calculator', () => {
  validateMeta(TG);
  validateInterface(TG);

  describe('Fee calculations (delegates to AP)', () => {
    runBenchmarkTests(TG, benchmarkTests);
  });

  describe('TG-specific metadata', () => {
    test('should have TG as stateCode', () => {
      expect(TG.META.stateCode).toBe('TG');
    });

    test('should reference AP Act as adopted', () => {
      expect(TG.META.legislation.act).toContain('Adopted');
    });
  });

  describe('Input validation', () => {
    test('Rejects missing suit value', () => {
      const { isValid } = TG.validateInput();
      expect(isValid).toBe(false);
    });

    test('Rejects negative suit value', () => {
      const { isValid } = TG.validateInput(-100);
      expect(isValid).toBe(false);
    });
  });
});
