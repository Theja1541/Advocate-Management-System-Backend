...
const getCaseReport = async (options = {}) => {
...
const getAdvocateReport = async (options = {}) => {
...
const getClientReport = async (options = {}) => {
  const fcC = getFilterCond(options, "created_at");
  const fcP = getFilterCond(options, "transaction_date");
  const rows = await sequelize.query(
    `
...
const getPaymentReport = async (options = {}) => {
...
const getMembershipReport = async (options = {}) => {
...
const getStateWiseReport = async (options = {}) => {
...
const generateReport = async (reportType, options = {}) => {
  ...
  switch (normalized) {
    case 'case':
      data = await getCaseReport(options);
      break;
    case 'advocate':
      data = await getAdvocateReport(options);
      break;
    case 'client':
      data = await getClientReport(options);
      break;
    case 'payment':
      data = await getPaymentReport(options);
      break;
    case 'membership':
      data = await getMembershipReport(options);
      break;
    case 'daily':
    ...
    case 'monthly':
      ...
      break;
    case 'state-wise':
      data = await getStateWiseReport(options);
      break;
    default:
